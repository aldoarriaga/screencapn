...
impl CaptureDocument {
    ...
    pub fn estimated_history_bytes(&self) -> usize {
        let mut bytes = std::mem::size_of::<Self>()
            + self.annotations.capacity() * std::mem::size_of::<Annotation>();
        for annotation in &self.annotations {
            bytes += match &annotation.kind {
                AnnotationKind::Text { text, .. } | AnnotationKind::Watermark { text, .. } => {
                    text.capacity()
                }
                AnnotationKind::Tag { label, .. } => label.capacity(),
                AnnotationKind::Pen { points } | AnnotationKind::PenArrow { points } => {
                    points.capacity() * std::mem::size_of::<crate::Point>()
                }
                _ => 0,
            };
        }
...
mod tests {
    ...
    #[test]
    fn history_estimate_counts_pen_point_storage() {
        let mut doc = CaptureDocument::new();
        let points = vec![crate::Point::new(1.0, 1.0); 128];
        doc.add_annotation(Annotation::new(
            0,
        ...
        assert!(doc.estimated_history_bytes() >= 128 * std::mem::size_of::<crate::Point>());
    }
}
