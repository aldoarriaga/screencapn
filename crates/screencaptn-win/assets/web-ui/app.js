(function () {
  ...
  let pendingRenderedRevision = null;
  ...
  function scheduleRender(nextState, parts, renderedRevision) {
    if (nextState) {
    ...
    if (Number.isFinite(renderedRevision)) {
      pendingRenderedRevision = Math.max(pendingRenderedRevision == null ? renderedRevision : pendingRenderedRevision, renderedRevision);
    }
    if (renderScheduled) {
      return;
    }
    ...
    requestAnimationFrame(() => {
      ...
      if (pendingRenderedRevision != null) {
        host({ type: "rendered", revision: pendingRenderedRevision });
        pendingRenderedRevision = null;
      }
    });
  }
  ...
  function uiRenderSignature(value) {
    ...
    return JSON.stringify({
      ...
      watermarkFontSize: value.watermarkFontSize,
  ...
  function clearLocalTextEditingState(id) {
    const baseState = pendingState || state;
    if (!baseState || baseState.editingTextId !== id) {
      return;
    }
    const nextState = {
      ...baseState,
    ...
    state = nextState;
    pendingState = null;
    scheduleRender(nextState, { committed: true, selection: true });
  }

  function commitTextEditor(copyAfterCommit, preserveEmpty) {
    ...
    destroyTextEditor(false);
    clearLocalTextEditingState(session.id);
    if (copyAfterCommit) {
  ...
  function drawToolbar(rect) {
    ...
    for (const item of tools) {
      ...
      const selected = (item.tool && state.activeTool === item.tool) || (item.action === "numbering" && state.activeTool === "step");
      if (selected) {
      ...
      if (item.action === "numbering") {
        ...
        button.on("click tap", () => {
          ...
          applyLocalDeselect();
          if (state) {
            const disable = state.numberingEnabled && state.activeTool === "step";
            state = {
              ...state,
              numberingEnabled: !disable,
              activeTool: disable ? "rectangle" : "step",
            };
            scheduleRender(state, { ui: true, selection: true, preview: true });
          }
        ...
        if (item.tool) {
          button.on("click tap", () => {
            ...
            applyLocalDeselect();
            if (state) {
              state = { ...state, activeTool: item.tool };
              scheduleRender(state, { ui: true, selection: true, preview: true });
            }
  ...
  function submenuLayout(toolbar, tool, s) {
    const width = submenuContentWidth(tool) * s;
    const height = 24 * s;
  ...
  function submenuContentWidth(tool) {
    const edge = 8;
    const divider = 10;
    const iconButton = 28;
    const strokeSlider = 116;
    const fontSlider = 122;
    const textInput = 132;
    const swatches = colors.length * 21 + 2;
    switch (tool) {
      case "rectangle":
      case "oval":
      case "line":
      case "arrow":
        return edge * 2 + strokeSlider + divider + swatches;
      case "pen":
      case "highlighter":
        return edge * 2 + iconButton * 2 + divider * 2 + strokeSlider + swatches;
      case "text":
        return edge * 2 + iconButton * 2 + divider * 2 + fontSlider + swatches;
      case "tag":
        return edge * 2 + strokeSlider + divider + fontSlider + divider + swatches;
      case "watermark":
        return edge * 2 + iconButton * 3 + divider * 2 + fontSlider + textInput + swatches;
      case "step":
        return edge * 2 + iconButton * 2;
      default:
        return 0;
    }
  }
  ...
  function drawOptionIcon(group, x, icon, selected, onClick, p, s) {
    ...
    group.add(item);
    item.add(new Konva.Rect({ x: 0, y: 0, width: box, height: box, cornerRadius: 2 * s, fill: selected ? p.selected : "rgba(0,0,0,0)", name: "ui" }));
  ...
  function selectedHandleAtPoint(point) {
    ...
    const x1 = selectionBounds.x;
    const x2 = selectionBounds.x + selectionBounds.width / 2;
    const x3 = selectionBounds.x + selectionBounds.width;
  ...
  function annotationHitTest(point) {
    return annotationAtPoint(point) != null;
  }
  ...
  function annotationContainsPoint(annotation, point) {
    ...
    if (kind.type === "text" && !kind.framed) {
      return pointInRect(point, inlineTextHitBounds(bounds, kind), physicalScalar(10));
    }
    return pointInRect(point, bounds, kind.type === "text" ? physicalScalar(14) : 4 * scale());
  }
  ...
  function inlineTextHitBounds(bounds, kind) {
    return inlineTextLayout(bounds, kind).bounds;
  }

  function inlineTextLayout(bounds, kind) {
    const fontSize = physicalScalar(kind.fontSize || state.fontSize || 27);
    const padding = physicalScalar(8);
    const lines = String(kind.text || "").split(/\r?\n/);
    const lineWidths = lines.map((line) => measureTextWidth(line, fontSize, false));
    const width = Math.max(fontSize, ...lineWidths);
  ...
  stage.on("mousedown touchstart", (evt) => {
    ...
    if (textEditorSession) {
      ...
      const keepsCurrentAnnotation =
        (selectedHandle && selectedHandle.id === editingId) ||
        (targetAnnotation && targetAnnotation.id === editingId);
      commitTextEditor(false, keepsCurrentAnnotation);
    }
    if (selectedHandle) {
      preview = null;
      command("beginAnnotationEdit", {
        ...payload,
        id: selectedHandle.id,
        editHandle: selectedHandle.key,
      });
      schedulePreviewRender();
      return;
    }
  ...
  function upsertCommittedAnnotation(annotation) {
    ...
    if (!cached || cached.signature !== signature) {
      ...
      if (cached) {
        group.zIndex(cached.group.zIndex());
        cached.group.destroy();
      }
  ...
  function drawCommittedText(annotation, kind, bounds, color) {
    ...
    if (framed) {
      if (filled) {
        addCommitted(new Konva.Rect({ x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height, cornerRadius: physicalScalar(6), fill: color }));
      }
      ...
      const inlineLayout = inlineTextLayout(bounds, kind);
      const textY = bounds.y - fontSize * 1.08;
      const lineHeight = inlineLayout.lineHeight;
      const inlineLines = inlineLayout.lines;
      if (filled && text) {
        inlineLines.forEach((line, index) => {
          const lineWidth = Math.max(fontSize * 0.45, inlineLayout.lineWidths[index] || 0);
          addCommitted(new Konva.Rect({
            x: bounds.x - padding,
            y: textY + index * lineHeight - padding * 0.45,
            width: lineWidth + padding * 2,
            height: lineHeight + padding * 0.45,
            cornerRadius: physicalScalar(3.5),
            fill: color,
          }));
        });
      }
  ...
  function drawCommittedTag(annotation, kind, bounds, color) {
    ...
    const radius = physicalScalar(style.tagRadius || 5);
  ...
  function drawSelection(annotation) {
    ...
    if (kind.type === "line" || kind.type === "arrow" || (kind.type === "highlighter" && kind.shape === "line")) {
      ...
      if (kind.type === "text") {
        ...
        drawHandle({ x: selectionBounds.x, y: selectionBounds.y + selectionBounds.height / 2 }, color, true);
      }
      if (kind.type === "tag") {
  ...
  function drawHandle(point, color, compact) {
    const size = compact ? Math.max(5, physicalScalar(6)) : Math.max(6, physicalScalar((renderStyle().selectionHandleSize || 8)));
  ...
  function autoStepBadgeCenter(annotation) {
    const kind = annotation.kind || {};
    const size = physicalScalar(renderStyle().stepBadgeSize || 24);
    const unit = size / 24;
    if (kind.type === "line" || kind.type === "arrow" || (kind.type === "highlighter" && kind.shape === "line")) {
      const start = physicalToCssPoint(kind.start);
      const end = physicalToCssPoint(kind.end);
      return {
        x: (start.x + end.x) / 2,
        y: (start.y + end.y) / 2 - strokeWidth(annotation) / 2 - size / 2 - 6 * unit,
      };
    }
    if (kind.type === "pen" || kind.type === "penArrow") {
      ...
      if (midpoint) {
        return {
          x: midpoint.x,
          y: midpoint.y - strokeWidth(annotation) / 2 - size / 2 - 3 * unit,
        };
      }
    }
    const bounds = physicalToCssRect(annotation.bounds);
  ...
  function stepBadgeHitTest(point) {
    ...
    const radius = physicalScalar(renderStyle().stepBadgeSize || 24) / 2 + 4 * scale();
  ...
  function renderPreview() {
    ...
    if (tool === "rectangle") {
      ...
      if (state.highlighterShape === "line") {
        ...
        getPreviewNode("highlighter-area", () => new Konva.Rect()).setAttrs({ x: rect.x, y: rect.y, width: rect.width, height: rect.height, cornerRadius: 4 * scale(), fill: color, opacity: alpha });
      }
    } else if (tool === "mosaic") {
      hidePreviewLayer();
      return;
      ...
      const size = physicalScalar(style.stepBadgeSize || 24);
      const fontSize = physicalScalar(style.stepBadgeFontSize || 14);
      group.add(new Konva.Circle({ x: start.x, y: start.y, radius: size / 2, fill: color }));
      group.add(
        new Konva.Text({
          x: start.x - size / 2,
          y: start.y - size / 2,
          ...
          verticalAlign: "middle",
          lineHeight: 1,
          text: String(state.nextStepNumber || 1),
          fontFamily: "Segoe UI",
          fontStyle: "700",
          fontSize,
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (
      ...
      (state.selectedAnnotationId != null ||
        ...
        state.editingStepNumberId != null ||
        state.editingWatermarkText)
    ) {
      applyLocalDeselect();
    }
  ...
  function applyRenderDiff(diff) {
    ...
    const watermarkChanged = ["watermarkText", "watermarkColor", "watermarkImageUrl", "watermarkImageDataUrl", "watermarkDateEnabled", "watermarkMode", "watermarkFontSize"].some((key) => Object.prototype.hasOwnProperty.call(patch, key));
  ...
  window.chrome.webview.addEventListener("message", (event) => {
    ...
    if (event.data.type === "state") {
      scheduleRender(event.data.state, undefined, event.data.revision);
